...
def analyze_gold_data():
    ...
    print(
        f"Forms with gaps in alignment: {alignments_with_gaps:,} ({100 * alignments_with_gaps / total_forms:.1f}%)"
    )
    ...
    for cognacy, forms in cognate_sets.items():
        has_gaps = any(
            " - " in f["alignment"]
            or f["alignment"].startswith("- ")
            or f["alignment"].endswith(" -")
            for f in forms
        )
    ...
    for dataset, good_sets in sorted(dataset_quality.items(), key=lambda x: x[1], reverse=True)[
        :10
    ]:
